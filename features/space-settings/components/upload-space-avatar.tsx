...
import { upsertSpaceAvatar } from "../api/upsert-space-avatar";
import { v4 as uuidv4 } from "uuid";

type UploadSpaceAvatarProps = {
...
export function UploadSpaceAvatar({
  ...
  const getCropData = async () => {
    startTransition(async () => {
      const previousImage = spaceAvatar ? spaceAvatar.file_name : null;
      if (cropper) {
        ...
        if (file) {
          ...
          if (data) {
            ...
            if (publicUrl) {
              upsertSpaceAvatar(spaceId, publicUrl.publicUrl, filename);
